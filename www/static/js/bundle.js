...
async function mapOpenBankingAccount(accountId, mappedHabitAccountId, owner, balanceType) {
  try {
    const payload = {
...
function getOverviewTileConfig() {
  const cfg = getSettings();
...
async function saveOverviewTilePreferences(cfg) {
  try {
...
function flipForecastTile(tileId) {
...
function handleForecastTilePointerDown(event, tileId) {
...
function handleForecastTilePointerMove(event) {
...
function handleForecastTilePointerUp(event, tileId) {
  if (longPressTimer) {
    clearTimeout(longPressTimer);
    longPressTimer = null;
  }
...
function handleForecastTilePointerCancel(event, tileId) {
  if (longPressTimer) {
    clearTimeout(longPressTimer);
    longPressTimer = null;
  }
...
function handleForecastTileClick(event, tileId, target) {
  if (appState.globalEditMode) return;
...
function reorderOverviewTiles(sourceTileId, targetTileId) {
...
function onForecastTileDragStart(event, tileId) {
...
function onForecastTileDragOver(event, tileId) {
...
function onForecastTileDragEnter(event, tileId) {
  event.preventDefault();
...
function onForecastTileDragLeave(event, tileId) {
  const el = document.getElementById(`tile-wrap-${tileId}`);
...
function onForecastTileDrop(event, targetTileId) {
  event.preventDefault();
...
function onForecastTileDragEnd(event) {
  draggedTileId = null;
  document.querySelectorAll('.tile-drag-over').forEach(el => el.classList.remove('tile-drag-over'));
  document.querySelectorAll('.tile-dragging').forEach(el => el.classList.remove('tile-dragging'));
}
...
function onTouchDragStart(event, tileId) {
...
function moveOverviewTileOrder(tileId, direction) {
...
function toggleOverviewTileVisibility(tileId, isVisible) {
  const cfg = getSettings();
  const { allOrder, visibleTiles } = getOverviewTileConfig();
...
function toggleOverviewTileExpansion(tileId) {
  const cfg = getSettings();
...
function resetOverviewTilesToDefault() {
  const cfg = getSettings();
  const allTileIds = FORECAST_OVERVIEW_TILES.map(t => t.id);
...
function openOverviewTilesModal() {
  const { allOrder, visibleTiles, expandedTiles } = getOverviewTileConfig();
...
function filterOverviewTilesModal(query) {
...
function enableHomeAssistantKioskMode() {
...
function scrollToActiveMonthPill(smooth = true) {
  setTimeout(() => {
